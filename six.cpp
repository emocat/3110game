...
void Six::queryUser(Question q)
    {
        ...
        char userAnswer[100];
        char lowerCase[100];
        char * test; //pointer to first occurrence of userAnswer in q.answer
        char reply[2];
        int length;
        cout << "Would you like to solve a clue? y/n" << endl;
        cin.clear();
        cin.getline(reply, 2);
        if (strcmp(reply,yes) == 0){
            ...
            while (q.solved == false && q.attemptsLeft >0)
            {
                for(int i = 0; i<25; i++){
                    ...
                    test = strstr(userAnswer,q.answer); //if user's answer is found WITHIN the correct answer                
                    }
                if (strcmp(userAnswer,q.answer)==0 || test != 0)
                {
                    cin.clear();
                    score++;
                    q.solved = true;
...
void Six::printDirections()
    {
        ...
        cout << "Enter 'skip' to skip a question. Enter 'give up' to see the answer." << endl;
        cout << "You will score one point for each correct zero points."<< endl;
        cout << "No points will be awarded for skipping or giving up on a question." << endl; 
